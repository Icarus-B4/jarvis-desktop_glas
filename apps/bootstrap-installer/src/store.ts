...
/// desktop app handing off via `J.A.R.V.I.S.-Setup.exe --update`.
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.jarvis/logs/bootstrap-installer.log')
    $jarvisHome.set('~/.jarvis')
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch J.A.R.V.I.S." success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (J.A.R.V.I.S.-Setup.exe --update);
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'J.A.R.V.I.S. repository' },
