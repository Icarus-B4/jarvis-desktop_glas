...
export type VoiceOrb3DProps = {
  levels?: number[];
  paused?: boolean;
  className?: string;
};

export function VoiceOrb3D({ levels = [], paused = false, className = "" }: VoiceOrb3DProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const render = () => {
      try {
        ...
        console.warn("[VoiceOrb3D] Render cycle caught error:", err);
      }
